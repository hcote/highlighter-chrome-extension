...
function applyHighlights(pageHighlights) {
    ...
    for (key in pageHighlights) {
        ...
        for (let i = 0; i < nodeList.length; i++) {            
            if (pageHighlights[key][1] === nodeList[i].innerText.indexOf(key) || pageHighlights[key][2] === nodeList[i].innerText.indexOf(key)) {
                grabNoteIfExists(pageHighlights);
                nodeList[i].innerHTML = nodeList[i].innerHTML.replace(key, `<span style="background-color: rgb(199, 255, 216);" class="el" title="${note}">`+key+'</span>');
            }
        }
    }
}
